...
<div class="container">
	<div class="row">
		<?php $this->load->view("left_menu",array("left"=>!empty($_GET['option']) ? $_GET['option']:"customer")); ?>
		<div id="subRight" class="col-md-9">
			<div class="padgeName">
				<h2><?= ($this->input->get("bbc_code") == 4 && $this->input->get("option") == "my") ? "Q&A" : $panel[0]->title?></h2>
			</div>
			<div class="con">
				<form method="post" name="frmBbs" id="frmBbs" enctype="multipart/form-data" action="/bbSend">
					...
					<div class="row my-4">
						<div class="col-md-12">
							...
							<?= empty($content) ? $panel[0]->content : $content ?>
							</textarea>
						</div>
					</div>
					<div class="row">
                        <div class="col-md-6">
                            ...
                            <?php if(!empty($file1)): ?>
                            <a href="javascript:void(0)" onclick="deleteFile('<?=$file1?>',1,this)" class="text-danger"><?=$file1?> 삭제</a>
                            <div id="file1"></div>	
                            <?php endif; ?>
                            <?php if(empty($file1)): ?>
					...
					<div class="row my-4">
						<div class="col-xs-12 center">
							...
							value="<?=empty($this->input->get("board_id")) ? "등록" : "수정"?>">
							<a href="panel?id=<?=$panel[0]->iden?><?=!empty($_GET['option']) ? "&option=".$_GET['option']:"" ?>" class="btn btn-warning btn-round">취소</a>
						</div>
					</div>
				</form>
			</div>
...
<script>
    tinymce.init({
       ...
       content_css: "/assets/dist/css/editor.css",
...
<script>
	function deleteFile(url,type,obj){
		...
		$.ajax({
	        ...
	        data:{url:url,id:<?=empty($this->input->get("board_id")) ? 0 : $this->input->get("board_id")?>,type:type},
	        success:function(data){            	
	            if(data ==0) alert("서버오류");
	            else
	            {
	            	$("#file"+type).html(" <input type='file' name='file"+type+"' class='my-4'>");
	            	$this.remove();
	            }
	        },
	        error: function(data){
